module "logs" {
  source                   = "git::https://github.com/techieBrandon/terraform-aws-s3-log-storage.git?ref=master"
  name                     = "${var.name}"
  stage                    = "${var.stage}"
...
module "dns" {
  source           = "git::https://github.com/techieBrandon/terraform-aws-route53-alias.git?ref=master"
